...
class OurGroupList extends Component
{
    ...
    public int $groupId, $group_leader_id, $group_type = 0;
    ...
    public function render()
    {
        if ($this->group_type == 1) {
            $groups = Auth::user()->groups()->search($this->search)->with(['groupLeader'])->latest()->paginate(10);
        } else {
            $groups = Group::search($this->search)->where('group_leader_id', auth()->user()->id)->with(['groupLeader'])->latest()->paginate(10);
        }
        $users = User::all(['id as value', 'fName', 'lName', 'email']);


        $title = "لیست گروه های " . auth()->user()->fullName;
        return view('livewire.support.group.our-group-list', [
            'groups' => $groups,
            'users' => $users->toArray(),
        ])->title($title);
    }
    ...
    {
        if ($group->groupLeader->id != auth()->user()->id) {
            abort(403);
        }
